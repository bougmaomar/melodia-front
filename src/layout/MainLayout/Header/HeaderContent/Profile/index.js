...
// assets
import avatar1 from 'assets/images/users/default.jpeg';
...
const ProfilePage = () => {
  ...
  // State to manage popper open/close
  ...
  // Handle component state when user changes
  useEffect(() => {
    if (!user) {
      setOpen(false);
      navigate(`/login`);
  ...
  return (
    <Box sx={{ flexShrink: 0, ml: 0.75 }}>
      ...
      >
        <Avatar alt="profile user" src={user.photo ? `${API_MEDIA_URL}${user.photo}` : avatar1} />
      </ButtonBase>
      <Popper
